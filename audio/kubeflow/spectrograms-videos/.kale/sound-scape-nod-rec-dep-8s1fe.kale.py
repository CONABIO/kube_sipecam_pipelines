...
def spec_n_specvid(AUTH_ENDPOINT: str, BASE_ENDPOINT: str, BLUE_IDX: str, CUMULO: int, GREEN_IDX: str, RED_IDX: str, SPECTRUM: str, VIDS_PER_HOUR: int):
    ...
    SPECTRUM = "{}"
    VIDS_PER_HOUR = {}
    '''.format(AUTH_ENDPOINT, BASE_ENDPOINT, BLUE_IDX, CUMULO, GREEN_IDX, RED_IDX, SPECTRUM, VIDS_PER_HOUR)
    ...
    _kale_marshal.set_data_dir("/shared_volume/audio/.spec_specvid-sipecam-cumulus-node-recorder-deployment-aws.ipynb.kale.marshal.dir")
    ...
    def change_type_sipecam_sc(session, root_folder_id, path, file_type, node_type):
        ...
        try:
            ...
            response = session.get(
                ...
                + "/nodes/"
                + root_folder_id
                + "/children?relativePath="+semi_path+"&include=aspectNames&skipCount=0&maxItems=1000"
            ) 
                ...
                for entry in response_entries:
                    if entry['entry']['name']==file_type and entry['entry']['isFile']:
                        ...
                        if entry['entry']['name']==file_type:
                            print(f"Changing type for {os.path.join(semi_path)}/{entry['entry']['name']}")
                            
                            sc_products = ["sequence.png", "mean_soundscape.png", "std_soundscape.png"]
                            if entry['entry']['name'] in sc_products:
        ...
        if "hashed_soundscape.parquet" in os.listdir(sc_path):
            ...
            for id_audio in ids_audios:
                ...
                try:
                    ...
                    audio2video(id_audio, recs, sc_path, SPECTRUM, CUMULO)
                except:
                    pass

        else:
            print(f"hashed_soundscape.parquet not found in {sc_path}")
    '''
...
def upload_alfresco_model_data(ALFRESCO_NODE_ID: str, AUTH_ENDPOINT: str, BASE_ENDPOINT: str):
    ...
    for sc_path in sub_folder_results:
        for file in os.listdir(sc_path):
            if file.split(".")[-1] in ["png", "mp4"]:
                print(file)
                change_type_sipecam_sc(session, ALFRESCO_NODE_ID, sc_path, file, 'soundscape:product')
    '''
...
@_kfp_dsl.pipeline(
    name='sound-scape-nod-rec-dep-8s1fe',
    description='Computes Sipecam Soundscapes using cumulus, node, recorder and deployment'
)
def auto_generated_pipeline(ALFRESCO_NODE_ID='cf3a1b97-965d-489f-bfdf-5c8e26c4ac95', AUTH_ENDPOINT='alfresco/api/-default-/public/authentication/versions/1', BASE_ENDPOINT='alfresco/api/-default-/public/alfresco/versions/1', BLUE_IDX='CORE', CUMULO='32', FREQUENCY_BINS='96', FREQUENCY_LIMITS_LB='0', FREQUENCY_LIMITS_UB='24000', GREEN_IDX='INFORMATION', HASHER_TIME_MODULE='48', HASHER_TIME_UNIT='1800', HASH_NAME='crono_hash_30m', LIMIT='False', MIN_FREQ_SC='10000', PAGESIZE='4000', RED_IDX='EXAG', RESULTS_DIR='/shared_volume/audio/soundscapes', SAMPLERATE='48000.0', SPECTRUM='Audible', THREADS_PER_WORKER='2', TIME_UNIT='30', VIDS_PER_HOUR='3', WORK_DIR_PIPELINE='/shared_volume/audio', vol_shared_volume='hostpath-pvc'):
    ...
    _kale_spec_n_specvid_task = _kale_spec_n_specvid_op(AUTH_ENDPOINT, BASE_ENDPOINT, BLUE_IDX, CUMULO, GREEN_IDX, RED_IDX, SPECTRUM, VIDS_PER_HOUR)\
...
if __name__ == "__main__":    
    ...
    run_name = generate_run_name('sound-scape-nod-rec-dep-8s1fe')
    pipeline_parameters = {"CUMULO": int(args.cumulus),
                           "SAMPLERATE": 48000.0,
